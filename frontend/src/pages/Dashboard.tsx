...
export default function Dashboard() {
  ...
  const handleFetchX = async () => {
    try {
      ...
      setError(err instanceof Error ? err.message : 'Failed to fetch X bookmarks');
    } finally {
      ...
      <div className="container mx-auto px-4 py-8 max-w-6xl">
        ...
        <div className="bg-white rounded-lg shadow-md p-6 mb-8">
          ...
          <div className="flex gap-4">
            ...
            >
              {isFetchingX ? 'Fetching...' : 'Fetch X Bookmarks'}
            </button>
            <button
              onClick={handleFetchLinkedIn}
              disabled={isFetchingLinkedIn}
              className="flex-1 bg-blue-700 text-white py-2 px-4 rounded-lg hover:bg-blue-800 disabled:opacity-50 disabled:cursor-not-allowed transition-colors"
            >
